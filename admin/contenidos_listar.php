<?php	
	...
	$consulta = "select * from jp_contenidos order by jpcon_titulo asc";
	$consultar_contenidos = $conexion_bd->consultar_tabla($consulta, $link);
	$numero_total = $conexion_bd->numero_registros_consulta_tabla($consultar_contenidos);	
?>
...
<table id="tabla_listar">
    ...
    <?php
		$i = 1; 
		while($conexion_bd->recorrer_consulta_tabla($consultar_contenidos)) {
			...
			echo '<tr style="font-size: 11px;"><td>'.$jpcon_titulo.'</td><td><a href="contenidos_editar.php?id='.$jpcon_id.'" target="_parent"><img src="images/btn_editar.png" width="35" height="35" border="0" alt="Editar contenido" /></a></td></tr>';
			$i ++;
		}
	?>
    </table>
