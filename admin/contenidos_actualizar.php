<?php
	...
	if($actualizar == 1) {
		$campos_consulta = "";
		$condicion = "jpcon_id = $jpcon_id";
		
		if($jpcon_detalle != "") {
			if($campos_consulta != "")
				$campos_consulta .= ", jpcon_detalle = '".$jpcon_detalle."'";	
			else
				$campos_consulta .= "jpcon_detalle = '".$jpcon_detalle."'";		
		}			
				
		$consulta = "update jp_contenidos set $campos_consulta where $condicion";
		if(@$actualizar_contenidos = $conexion_bd->consultar_tabla($consulta, $link)) {
