<?php	
	...
	if($actualizar == 1) {
		$campos_consulta = "";
		$condicion = "jpban_id = $jpban_id";
		
		if($jpban_detalle != "") {
			if($campos_consulta != "")
				$campos_consulta .= ", jpban_detalle = '".$jpban_detalle."'";	
			else
				$campos_consulta .= "jpban_detalle = '".$jpban_detalle."'";		
		}	
		...
		if($jpban_url != "") {
			if($campos_consulta != "")
				$campos_consulta .= ", jpban_url = '".$jpban_url."'";	
			else
				$campos_consulta .= "jpban_url = '".$jpban_url."'";		
		}			
				
		$consulta = "update jp_banner set $campos_consulta where $condicion";
		if(@$actualizar_banners = $conexion_bd->consultar_tabla($consulta, $link)) {
