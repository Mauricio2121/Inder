<?php	
	...
	class conexion_bd {
		...
		function consultar_tabla($sql, $conexion) {			
			$this->resultado_consulta_tabla = mysql_query($sql, $conexion);			
		...
		function recorrer_consulta_tabla($resultado) {			
			$this->valores_consulta_tabla = mysql_fetch_array($resultado);
		...
		function numero_registros_consulta_tabla($resultado) {
			$this->cantidad_consulta_tabla = mysql_num_rows($resultado);
