<?php
	...
	$consulta = "select * from jp_usuarios where jpusu_usuario = '$usuario' and jpusu_clave = '$clave' and jpusu_estado = 1";
	$consultar_usuarios = $conexion_bd->consultar_tabla($consulta, $link);
	if($conexion_bd->recorrer_consulta_tabla($consultar_usuarios)) {
		$id_usuario = $conexion_bd->mostrar_campo_consulta_tabla("jpusu_id");
		$nombre_usuario = $conexion_bd->mostrar_campo_consulta_tabla("jpusu_nombre");
		$email_usuario = $conexion_bd->mostrar_campo_consulta_tabla("jpusu_email");
		$nivel_acceso = 1;				
		...
		header("Location:../index.php?usuario_invalido=si"); 
	}
	ob_end_flush();
?> 
