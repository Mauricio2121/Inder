<?php	
	...
	if($actualizar == 1) {
		$campos_consulta = "";
		$condicion = "jpcom_id = $jpcom_id";
		
		if($jpcom_detalle != "") {
			if($campos_consulta != "")
				$campos_consulta .= ", jpcom_detalle = '".$jpcom_detalle."'";	
			else
				$campos_consulta .= "jpcom_detalle = '".$jpcom_detalle."'";		
		}			
		if($jpcom_autor != "") {
			if($campos_consulta != "")
				$campos_consulta .= ", jpcom_autor = '".$jpcom_autor."'";
			else
				$campos_consulta .= "jpcom_autor = '".$jpcom_autor."'";		
		}			
		
		$consulta = "update jp_comentarios set $campos_consulta where $condicion";
		if(@$actualizar_comentarios = $conexion_bd->consultar_tabla($consulta, $link)) {
